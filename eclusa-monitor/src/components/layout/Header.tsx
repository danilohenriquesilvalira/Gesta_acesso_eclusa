...
import type { ServidorHealth } from "../../types";

type Pagina = "dashboard" | "admin-usuarios" | "admin-logs" | "admin-blacklist" | "admin-servidores" | "rede";
...
const RESERVAS = [
  { id: "Reserva01", label: "R1" },
  { id: "Reserva02", label: "R2" },
  { id: "Reserva03", label: "R3" },
];

export default function Header({ utilizador, isAdmin, apiOk, pagina, onPagina, onLoginClick, onSair, servidorHealth = {} }: Props) {
      ...
      <div className="relative flex items-stretch h-[68px] px-8">
        ...
        <div className="flex items-center gap-5 pr-8" style={{ borderRight: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div className="flex items-center gap-2 pl-3" style={{ borderLeft: "1px solid rgba(255,255,255,0.07)" }}>
            {RESERVAS.map(({ id, label }) => {
              const h     = servidorHealth[id];
              const pronto = h?.windows_vivo && h?.wincc_vivo;
              const semSinal = !h || (!h.windows_vivo && !h.wincc_vivo);
              return (
                <div key={id} className="flex flex-col items-center gap-1" title={`${id}: ${pronto ? "Pronto" : "Indisponível"}`}>
                  <span
                    className="w-2.5 h-2.5 rounded-full"
                    style={{
                      background: pronto ? "#28FF52" : "#E32C2C",
                      boxShadow:  pronto ? "0 0 6px #28FF52" : "0 0 6px #E32C2C",
                      animation:  pronto ? "none" : "pulse-dot 1.2s ease-in-out infinite",
                      opacity:    semSinal && apiOk !== true ? 0.3 : 1,
                    }}
                  />
                  <span className="text-[9px] font-extrabold tracking-wider" style={{ color: pronto ? "rgba(40,255,82,0.6)" : "rgba(227,44,44,0.6)" }}>
                    {label}
                  </span>
                </div>
              );
