...
impl axum::extract::FromRequestParts<Shared> for AuthUser {
    ...
    async fn from_request_parts(parts: &mut Parts, state: &Shared) -> Result<Self, Self::Rejection> {
        ...
        // Verificar se token foi revogado (logout explícito ou bloqueio de conta)
        ...
        // Verificar se admin forçou logout (todos os tokens antes deste timestamp são inválidos)
