...
use crate::{config::Config, types::{now, RdpInfo}};

/// Chama qwinsta no servidor RDP remoto e devolve o estado actual da sessão.
/// Em Linux retorna sempre inacessível (sem qwinsta).
...
/// Obtém o IP do cliente RDP via WTS API (Windows only).
#[cfg(windows)]
pub fn obter_ip_cliente_rdp(server_ip: &str, session_id: u32, _cfg: &Config) -> Option<String> {
    ...
    unsafe {
        ...
        let ip = if ok.is_ok() && !buf.is_null() {
            ...
            eprintln!("[{}] WTSQuery falhou em {} sessão {}", now(), server_ip, session_id);
            None
        };
...
#[cfg(not(windows))]
pub fn obter_ip_cliente_rdp(_: &str, _: u32, _: &Config) -> Option<String> { None }
